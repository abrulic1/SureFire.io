...
const Profile = () => {
  ...
  const { data: transactions, isFetching } = useQuery('transactions', () => getUserTransactions(localStorage.getItem('account')), { staleTime: 6000 });

  return (
    <>
      <Header />
      <div className={ProfileStyles["profile-card"]}>
        {isFetching ? (   <div className={ProfileStyles.loading}>
